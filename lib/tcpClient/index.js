...
export default class TCPClient {
  constructor(config) {
    ...
    if (config.gossipSeeds) {
      assert(
        Array.isArray(config.gossipSeeds),
        `${baseErr}gossipSeeds must be an array`
      );
      assert(
        config.gossipSeeds.length >= 3,
        `${baseErr}at least 3 gossipSeeds are required`
      );
    ...
    assert(
      ...
      `${baseErr}credentials.username property not provided`
    );
    assert(
      config.credentials.password,
      `${baseErr}credentials.password property not provided`
    );
    ...
    if (_config.gossipSeeds && _config.gossipSeeds.length > 0) {
      _config.gossipSeeds = _config.gossipSeeds.map(
        seed =>
          new esClient.GossipSeed(
            { host: seed.hostname, port: seed.port },
            seed.hostHeader
          )
      );
    }
    ...
    this.createPersistentSubscription = createPersistentSubscription(_config);
    this.connectToPersistentSubscription = connectToPersistentSubscription(
      _config
    );
    this.deletePersistentSubscription = deletePersistentSubscription(_config);

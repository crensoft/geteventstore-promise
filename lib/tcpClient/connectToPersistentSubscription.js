...
export default (config) => (
	...
	const onConnected = async () => {
		try {
			const subscription = await connection.connectToPersistentSubscription(streamName, groupName, onEventAppeared, onDropped, new client.UserCredentials(config.credentials.username, config.credentials.password), bufferSize, autoAck);
